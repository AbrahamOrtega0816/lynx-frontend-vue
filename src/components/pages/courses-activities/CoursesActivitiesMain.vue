...
import { EActivities } from '/@src/common/enums/EActivities'
...
const activitie = ref(1)

const getActivitiesOfLesson = async () => {
  const response = await courseService
    .getActivitiesOfLesson(id)
    .then((res) => {
...
<template>
  ...
  <form class="wizard-v1-wrapper" @submit.prevent="() => wizard?.validateStepFn?.()">
    ...
    <FillInTheFieldsActivities
      :data="activities"
      v-if="activitie === EActivities.fillInTheFieldsActivities"
    />
